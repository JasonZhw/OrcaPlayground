"""Rebuild the demo layout, spawn G1, and run the existing locomotion check."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="G1 运控冒烟验证：重建 Layout → 生成 G1 → ONNX + PD 行走")
    parser.add_argument("--addr", default="127.0.0.1:50051", help="OrcaLab gRPC 地址")
    parser.add_argument("--layout", type=Path, default=DEFAULT_LAYOUT_PATH, help="OrcaLab v3 Layout JSON")
    ...
    parser.add_argument("--spawn-yaw", type=float, default=0.0, help="G1 初始偏航角（度）")
    parser.add_argument("--num-steps", type=int, default=1000, help="50 Hz 控制步数；1000 步约 20 秒")
...
def _load_lesson7_components():
    ...
    return G1_FRAME_SKIP, G1_MODEL_XML, G1_TIME_STEP, G1PickLocomotionEnv, OnlineVerifier
...
def main() -> None:
    ...
    spawn_rotation = _yaw_quaternion_wxyz(args.spawn_yaw)

    print("[WARNING] 即将清空 OrcaLab 当前运行场景，并从 Layout JSON 重新生成场景。")
    print(f"[INFO] Layout: {args.layout.expanduser().resolve()}")
    print(f"[INFO] G1: {config.scene.robot_asset_path}")
    print(f"[INFO] Spawn: position={spawn_position}, yaw={args.spawn_yaw} deg")
    ...
    actor_count = publish_layout_with_g1(
        ...
        g1_rotation_wxyz=spawn_rotation,
    )
    print(f"[INFO] 已发布 {actor_count} 个 Layout 物体 + G1；等待仿真服务重启……")
    time.sleep(3.0)

    frame_skip, model_xml, time_step, locomotion_env_class, verifier_class = _load_lesson7_components()
    env = locomotion_env_class(
    ...
    verifier = verifier_class("Demo 1: G1 运控冒烟验证")
    try:
